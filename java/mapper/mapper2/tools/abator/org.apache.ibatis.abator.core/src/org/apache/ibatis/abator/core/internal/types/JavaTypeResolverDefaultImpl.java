/*
 *  Copyright 2005 The Apache Software Foundation
 *
 *  Licensed under the Apache License, Version 2.0 (the "License");
 *  you may not use this file except in compliance with the License.
 *  You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 *  Unless required by applicable law or agreed to in writing, software
 *  distributed under the License is distributed on an "AS IS" BASIS,
 *  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 *  See the License for the specific language governing permissions and
 *  limitations under the License.
 */
package org.apache.ibatis.abator.core.internal.types;

import java.math.BigDecimal;
import java.sql.Types;
import java.util.Date;
import java.util.Map;

import org.apache.ibatis.abator.core.api.JavaTypeResolver;
import org.apache.ibatis.abator.core.exception.UnsupportedDataTypeException;
import org.apache.ibatis.abator.core.internal.db.ColumnDefinition;

public class JavaTypeResolverDefaultImpl implements JavaTypeResolver {

	protected Map properties;

	public JavaTypeResolverDefaultImpl() {
		super();
	}

	/*
	 * (non-Javadoc)
	 * 
	 * @see org.apache.ibatis.generator.api.types.JavaTypeResolver#setProperties(java.util.Map)
	 */
	public void setProperties(Map properties) {
		this.properties = properties;
	}

	/*
	 * (non-Javadoc)
	 * 
	 * @see org.apache.ibatis.generator.api.types.JavaTypeResolver#initializeResolvedJavaType(org.apache.ibatis.generator.core.db.ColumnDefinition)
	 */
	public void initializeResolvedJavaType(ColumnDefinition cd)
			throws UnsupportedDataTypeException {
		boolean forceBigDecimals = "true".equalsIgnoreCase((String) properties //$NON-NLS-1$
				.get("forceBigDecimals")); //$NON-NLS-1$

		ResolvedJavaType type = new ResolvedJavaType();

		switch (cd.getJdbcType()) {
		case Types.ARRAY:
			type.setJdbcTypeName("ARRAY"); //$NON-NLS-1$
			type.setFullyQualifiedName(Object.class.getName());
			break;

		case Types.BIGINT:
			type.setJdbcTypeName("BIGINT"); //$NON-NLS-1$
			type.setFullyQualifiedName(Long.class.getName());
			break;

		case Types.BINARY:
			type.setJdbcTypeName("BINARY"); //$NON-NLS-1$
			type.setFullyQualifiedName(Object.class.getName());
			break;

		case Types.BIT:
			type.setJdbcTypeName("BIT"); //$NON-NLS-1$
			type.setFullyQualifiedName(Object.class.getName());
			break;

		case Types.BLOB:
			type.setJdbcTypeName("BLOB"); //$NON-NLS-1$
			// this isn't a "real" type, but it is what the ResolvedJavaType
			// class needs to operate properly
			type.setFullyQualifiedName("java.lang.byte[]"); //$NON-NLS-1$
			break;

		case Types.BOOLEAN:
			type.setJdbcTypeName("BOOLEAN"); //$NON-NLS-1$
			type.setFullyQualifiedName(Boolean.class.getName());
			break;

		case Types.CHAR:
			type.setJdbcTypeName("CHAR"); //$NON-NLS-1$
			type.setFullyQualifiedName(String.class.getName());
			break;

		case Types.CLOB:
			type.setJdbcTypeName("CLOB"); //$NON-NLS-1$
			type.setFullyQualifiedName(String.class.getName());
			break;

		case Types.DATALINK:
			type.setJdbcTypeName("DATALINK"); //$NON-NLS-1$
			type.setFullyQualifiedName(Object.class.getName());
			break;

		case Types.DATE:
			type.setJdbcTypeName("DATE"); //$NON-NLS-1$
			type.setFullyQualifiedName(Date.class.getName());
			break;

		case Types.DECIMAL:
			type.setJdbcTypeName("DECIMAL"); //$NON-NLS-1$
			if (cd.getScale() > 0 || cd.getLength() > 18 || forceBigDecimals) {
				type.setFullyQualifiedName(BigDecimal.class.getName());
			} else if (cd.getLength() > 9) {
				type.setFullyQualifiedName(Long.class.getName());
			} else if (cd.getLength() > 4) {
				type.setFullyQualifiedName(Integer.class.getName());
			} else {
				type.setFullyQualifiedName(Short.class.getName());
			}
			break;

		case Types.DISTINCT:
			type.setJdbcTypeName("DISTINCT"); //$NON-NLS-1$
			type.setFullyQualifiedName(Object.class.getName());
			break;

		case Types.DOUBLE:
			type.setJdbcTypeName("DOUBLE"); //$NON-NLS-1$
			type.setFullyQualifiedName(Double.class.getName());
			break;

		case Types.FLOAT:
			type.setJdbcTypeName("FLOAT"); //$NON-NLS-1$
			type.setFullyQualifiedName(Float.class.getName());
			break;

		case Types.INTEGER:
			type.setJdbcTypeName("INTEGER"); //$NON-NLS-1$
			type.setFullyQualifiedName(Integer.class.getName());
			break;

		case Types.JAVA_OBJECT:
			type.setJdbcTypeName("JAVA_OBJECT"); //$NON-NLS-1$
			type.setFullyQualifiedName(Object.class.getName());
			break;

		case Types.LONGVARBINARY:
			type.setJdbcTypeName("LONGVARBINARY"); //$NON-NLS-1$
			// this isn't a "real" type, but it is what the ResolvedJavaType
			// class needs to operate properly
			type.setFullyQualifiedName("java.lang.byte[]"); //$NON-NLS-1$
			break;

		case Types.LONGVARCHAR:
			type.setJdbcTypeName("LONGVARCHAR"); //$NON-NLS-1$
			type.setFullyQualifiedName(String.class.getName());
			break;

		case Types.NULL:
			type.setJdbcTypeName("NULL"); //$NON-NLS-1$
			type.setFullyQualifiedName(Object.class.getName());
			break;

		case Types.NUMERIC:
			type.setJdbcTypeName("NUMERIC"); //$NON-NLS-1$
			if (cd.getScale() > 0 || cd.getLength() > 18 || forceBigDecimals) {
				type.setFullyQualifiedName(BigDecimal.class.getName());
			} else if (cd.getLength() > 9) {
				type.setFullyQualifiedName(Long.class.getName());
			} else if (cd.getLength() > 4) {
				type.setFullyQualifiedName(Integer.class.getName());
			} else {
				type.setFullyQualifiedName(Short.class.getName());
			}
			break;

		case Types.OTHER:
			type.setJdbcTypeName("OTHER"); //$NON-NLS-1$
			type.setFullyQualifiedName(Object.class.getName());
			break;

		case Types.REAL:
			type.setJdbcTypeName("REAL"); //$NON-NLS-1$
			type.setFullyQualifiedName(Double.class.getName());
			break;

		case Types.REF:
			type.setJdbcTypeName("REF"); //$NON-NLS-1$
			type.setFullyQualifiedName(Object.class.getName());
			break;

		case Types.SMALLINT:
			type.setJdbcTypeName("SMALLINT"); //$NON-NLS-1$
			type.setFullyQualifiedName(Short.class.getName());
			break;

		case Types.STRUCT:
			type.setJdbcTypeName("STRUCT"); //$NON-NLS-1$
			type.setFullyQualifiedName(Object.class.getName());
			break;

		case Types.TIME:
			type.setJdbcTypeName("TIME"); //$NON-NLS-1$
			type.setFullyQualifiedName(Date.class.getName());
			break;

		case Types.TIMESTAMP:
			type.setJdbcTypeName("TIMESTAMP"); //$NON-NLS-1$
			type.setFullyQualifiedName(Date.class.getName());
			break;

		case Types.TINYINT:
			type.setJdbcTypeName("TINYINT"); //$NON-NLS-1$
			type.setFullyQualifiedName(Byte.class.getName());
			break;

		case Types.VARBINARY:
			type.setJdbcTypeName("VARBINARY"); //$NON-NLS-1$
			type.setFullyQualifiedName(Object.class.getName());
			break;

		case Types.VARCHAR:
			type.setJdbcTypeName("VARCHAR"); //$NON-NLS-1$
			type.setFullyQualifiedName(String.class.getName());
			break;

		default:
			throw new UnsupportedDataTypeException("Unsupported Type: "
					+ cd.getJdbcType());
		}

		cd.setResolvedJavaType(type);
	}
}
