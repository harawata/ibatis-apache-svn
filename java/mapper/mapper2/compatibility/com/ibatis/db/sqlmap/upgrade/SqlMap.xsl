     ...
     <xsl:element name="cacheModel">
       ...
       <xsl:choose>
         ...
         <xsl:otherwise>
           <xsl:if test="@implementation">
             <xsl:attribute name="type"><xsl:value-of select="@implementation"/></xsl:attribute>
           </xsl:if>
         </xsl:otherwise>
       </xsl:choose>
       ...
       <xsl:for-each select="flush-interval">
         <xsl:element name="flushInterval">
           <xsl:if test="@hours">
             <xsl:attribute name="hours"><xsl:value-of select="@hours"/></xsl:attribute>
           </xsl:if>
           <xsl:if test="@minutes">
             <xsl:attribute name="minutes"><xsl:value-of select="@minutes"/></xsl:attribute>
           </xsl:if>
           <xsl:if test="@seconds">
             <xsl:attribute name="seconds"><xsl:value-of select="@seconds"/></xsl:attribute>
           </xsl:if>
           <xsl:if test="@milliseconds">
             <xsl:attribute name="milliseconds"><xsl:value-of select="@milliseconds"/></xsl:attribute>
           </xsl:if>
         </xsl:element>
       </xsl:for-each>
       <xsl:for-each select="flush-on-execute">
         <xsl:element name="flushOnExecute">
           <xsl:if test="@statement">
             <xsl:attribute name="statement"><xsl:value-of select="@statement"/></xsl:attribute>
           </xsl:if>
         </xsl:element>
       </xsl:for-each>
     </xsl:element>
     ...
     <xsl:for-each select="property">
       <xsl:element name="result">
         ...
         <xsl:if test="@columnIndex">
           <xsl:attribute name="columnIndex"><xsl:value-of select="@columnIndex"/></xsl:attribute>
         </xsl:if>
         <xsl:if test="@mapped-statement">
           <xsl:attribute name="select"><xsl:value-of select="@mapped-statement"/></xsl:attribute>
         </xsl:if>
         <xsl:if test="@type">
           <xsl:attribute name="jdbcType"><xsl:value-of select="@type"/></xsl:attribute>
         </xsl:if>
         <xsl:if test="@javaType">
           <xsl:attribute name="javaType"><xsl:value-of select="@javaType"/></xsl:attribute>
         </xsl:if>
         <xsl:if test="@null">
           <xsl:attribute name="nullValue"><xsl:value-of select="@null"/></xsl:attribute>
         </xsl:if>
       </xsl:element>
     </xsl:for-each>
   </xsl:element>
...
<xsl:template match="mapped-statement|dynamic-mapped-statement">
   <xsl:element name="statement">
     ...
     <xsl:if test="@parameter-map">
       <xsl:attribute name="parameterMap"><xsl:value-of select="@parameter-map"/></xsl:attribute>
     </xsl:if>
     <xsl:if test="@parameter-class">
       <xsl:attribute name="parameterClass"><xsl:value-of select="@parameter-class"/></xsl:attribute>
     </xsl:if>
     <xsl:if test="@result-map">
       <xsl:attribute name="resultMap"><xsl:value-of select="@result-map"/></xsl:attribute>
     </xsl:if>
     <xsl:if test="@result-class">
       <xsl:attribute name="resultClass"><xsl:value-of select="@result-class"/></xsl:attribute>
     </xsl:if>
     <xsl:if test="@cache-model">
       <xsl:attribute name="cacheModel"><xsl:value-of select="@cache-model"/></xsl:attribute>
     </xsl:if>
