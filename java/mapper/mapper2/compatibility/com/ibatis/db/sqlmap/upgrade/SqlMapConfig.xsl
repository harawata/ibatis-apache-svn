...
<xsl:template match="settings">
   <xsl:element name="settings">
     <xsl:if test="@useFullyQualifiedStatementNames">
       <xsl:attribute name="useStatementNamespaces"><xsl:value-of select="@useFullyQualifiedStatementNames"/></xsl:attribute>
     </xsl:if>
     <xsl:if test="@cacheModelsEnabled">
       <xsl:attribute name="cacheModelsEnabled"><xsl:value-of select="@cacheModelsEnabled"/></xsl:attribute>
     </xsl:if>
     <xsl:if test="@useBeansMetaClasses">
       <xsl:attribute name="enhancementEnabled"><xsl:value-of select="@useBeansMetaClasses"/></xsl:attribute>
     </xsl:if>
     <xsl:if test="@maxTransactions">
       <xsl:attribute name="maxTransactions"><xsl:value-of select="@maxTransactions"/></xsl:attribute>
     </xsl:if>
     <xsl:if test="@maxExecute">
       <xsl:attribute name="maxRequests"><xsl:value-of select="@maxExecute"/></xsl:attribute>
     </xsl:if>
   </xsl:element>
</xsl:template>
    ...
    <xsl:element name="transactionManager">
       <xsl:choose>
         <xsl:when test="//settings/@useGlobalTransactions='true'">
           ...
           <xsl:element name="property">
             ...
             <xsl:attribute name="value"><xsl:value-of select="//settings/@userTransactionJndiName"/></xsl:attribute>
           </xsl:element>
         </xsl:when>
         <xsl:otherwise>
       ...
       <xsl:element name="dataSource">
         ...
         <xsl:for-each select="property[@name and @value]">
           <xsl:text>
           </xsl:text><property name="{@name}" value="{@value}"/><xsl:text>
           </xsl:text>
         </xsl:for-each>
       </xsl:element>
       <xsl:text>
